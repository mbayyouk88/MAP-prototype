// PAREA Combined — Screens 23-33 (Step 5-8 + Cert)
...
function S23_AdjGrid({ navigate }) {
  ...
  return (
    <div>
      <PageHeader breadcrumb={['Step 5', 'Section XV.A']} title="Adjustment grid"
      ...
      <div style={{ marginBottom: 14 }}>
        <TextbookCallout topic="adjustments" why="Every adjustment needs market evidence — paired sales, matched pairs, sensitivity. Refresh before you grid."/>
      </div>

      <WorkfileCaptureButton step="p5" label="Adjustment grid · paired-sales derivation worksheet" source="True Footage + your derivation"/>
...
function S24_Cost({ navigate }) {
  return (
    <div>
      <PageHeader breadcrumb={['Step 5', 'Section XV.B']} title="Cost approach"
...
function S25_Income({ navigate }) {
  return (
    <div style={{ maxWidth: 880, margin: '0 auto' }}>
      <PageHeader breadcrumb={['Step 5', 'Section XV.C']} title="Income approach — defend the skip"
...
function S26_MR5({ navigate }) {
  return (
    <div>
      <PageHeader breadcrumb={['Step 5', 'Mentor Review 5']} title="Mentor Review 5"
        subtitle="Asynchronous review by James Mendel, MAI. Submit when you're ready — 24 hr typical turnaround."/>

      <div style={{ display: 'grid', gridTemplateColumns: '1.3fr 1fr', gap: 16 }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <ManualChecklist
            title="Self-check before submitting Step 5"
            subtitle="You mark these yourself. Mentor reviews your marks + the actual work."
            storageKey="mr5-selfcheck"
            items={[{"label":"Adjustment grid: each adjustment supported by market evidence","hint":"Paired sales, matched pairs, sensitivity — not industry rule of thumb."},{"label":"Net adjustments ≤ 15% · gross adjustments ≤ 25% (FNMA guidelines)","hint":"If you bust either, the comp is too dissimilar or the adjustment is wrong."},{"label":"Cost approach supported with M&S data and shown depreciation math","hint":"Effective age ÷ economic life × RCN."},{"label":"Income approach excluded only with written defense","hint":"Cite USPAP SR 1-4(c)."},{"label":"Reviewed McKissock textbook references for adjustments support","hint":"Boards specifically test for unsupported adjustments."}]}
          />
          <WorkfileCheck
            step="p5"
            mentorReviewN={5}
            navigate={navigate}
            expectedItems={[{"label":"Paired-sales derivation worksheet","match":"adjustment","hint":"Show how you derived each adjustment."},{"label":"Cost approach worksheet (Marshall & Swift)","match":"cost","hint":"M&S category, RCN, depreciation."},{"label":"Income approach skip rationale","match":"income","hint":"Written defense if income was excluded."}]}
          />
          <div style={{ display: 'flex', gap: 10 }}>
            <Button variant="outline" fullWidth onClick={() => navigate('adjustment-grid')}>Revise first</Button>
            <Button variant="primary" fullWidth onClick={() => navigate('reconciliation')}>Submit to mentor (async · 24 hr)</Button>
          </div>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <Card padding="14px 16px" style={{ background: '#fafbfc' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: 10.5, color: '#888' }}>Reviews Step 5 — returns scored 4-pt rubric</div>
              </div>
...
function S27_Reconciliation({ navigate }) {
  return (
    <div>
      <PageHeader breadcrumb={['Step 6', 'Section XVI']} title="Reconciliation"
        subtitle="Three indicators · one final value. Weighting must be quantitative reasoning, not 'I averaged them'."/>

      <div style={{ marginBottom: 14 }}>
        <TextbookCallout topic="reconciliation" why="Reconciliation is judgment, not arithmetic — re-read why averaging will fail mentor review."/>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: 14 }}>
        ...
        <Card padding={20}>
          ...
          <Button variant="primary" fullWidth onClick={() => navigate('mentor-review-6')} style={{ marginTop: 14 }}>
            Submit Step 6 · MR 6 <Icon name="arrow-right" size={13} color="#fff"/>
          </Button>
        </Card>
      </div>
    </div>
...
function S28_MR6({ navigate }) {
  return (
    <div>
      <PageHeader breadcrumb={['Step 6', 'Mentor Review 6']} title="Mentor Review 6"
        subtitle="Asynchronous review by James Mendel, MAI. Submit when you're ready — 24 hr typical turnaround."/>

      <div style={{ display: 'grid', gridTemplateColumns: '1.3fr 1fr', gap: 16 }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <ManualChecklist
            title="Self-check before submitting Step 6"
            subtitle="You mark these yourself. Mentor reviews your marks + the actual work."
            storageKey="mr6-selfcheck"
            items={[{"label":"Final value is within the indicator range","hint":"Not below the lowest indicator, not above the highest."},{"label":"Weighting is reasoned, not averaged","hint":"SR 1-5 — explain why one approach got more weight."},{"label":"Rounding is disclosed","hint":"$488,000 from $487,500: state it."},{"label":"Effective date matches the inspection date","hint":"Or extraordinary assumption documented."},{"label":"Reviewed McKissock textbook references for reconciliation","hint":"Boards specifically test for averaging."}]}
          />
          <WorkfileCheck
            step="p6"
            mentorReviewN={6}
            navigate={navigate}
            expectedItems={[{"label":"Reconciliation worksheet (your weighting + reasoning)","match":"reconciliation","hint":"Your weighting in writing."},{"label":"Final value narrative","match":"reconciliation","hint":"Why this number, not the contract price."}]}
          />
          <div style={{ display: 'flex', gap: 10 }}>
            <Button variant="outline" fullWidth onClick={() => navigate('reconciliation')}>Revise first</Button>
            <Button variant="primary" fullWidth onClick={() => navigate('report-writing')}>Submit to mentor (async · 24 hr)</Button>
          </div>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <Card padding="14px 16px" style={{ background: '#fafbfc' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 12.5, fontWeight: 700 }}>{MENTOR.name}</div>
                <div style={{ fontSize: 10.5, color: '#888' }}>Reviews Step 6 — returns scored 4-pt rubric</div>
              </div>
              <Badge color="warning">3 in queue</Badge>
            </div>
          </Card>
...
function S29_ReportWriting({ navigate }) {
  return (
    <div>
      <PageHeader breadcrumb={['Step 7', 'Section XIX']} title="URAR report drafting"
        subtitle="True Footage URAR form · auto-populated from your work · narrative sections you write · AI mentor pre-screen."/>

      <WorkfileCaptureButton step="p7" label="URAR draft · current revision" source="True Footage URAR Form 1004"/>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.6fr 1fr', gap: 14 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <ManualChecklist
            title="Report writing · self-check"
            subtitle="Mark each yourself as you write — no auto-grade."
            storageKey="reportwrite-selfcheck"
            items={[
              { label: "URAR form completion 100%", hint: "Every field — leave nothing blank or 'N/A' without a reason." },
              { label: "Reporting standard cited (URAR Form 1004)", hint: "SR 2-2 requires you name the report option." },
              { label: "Certification drafted", hint: "SR 2-3 — your name + signature attribution." },
              { label: "Signature affixed", hint: "Electronic signature, dated." },
              { label: "Reconciliation narrative present", hint: "SR 1-6 — explain which approach got most weight and why." },
            ]}/>
...
function S30_Workfile({ navigate }) {
  return (
    <div>
      <PageHeader breadcrumb={['Step 7', 'Section XX']} title="Workfile assembly"
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: 14 }}>
        ...
        <Card padding={20}>
          ...
          <Button variant="primary" fullWidth onClick={() => navigate('mentor-review-7')} style={{ marginTop: 14 }}>
            Submit Step 7 · MR 7 <Icon name="arrow-right" size={13} color="#fff"/>
          </Button>
        </Card>
      </div>
    </div>
...
function S31_MR7({ navigate }) {
  return (
    <div>
      <PageHeader breadcrumb={['Step 7', 'Mentor Review 7']} title="Mentor Review 7 — Report & Workfile"
        subtitle="Asynchronous review of your URAR report and workfile assembly. Before you submit: walk the static USPAP self-check yourself."/>

      <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: 16 }}>
        {/* Static USPAP checklist — the centerpiece */}
        <div>
          <StaticUSPAPChecklist title="USPAP self-check · mark each yourself" printable/>
          <Card padding="14px 16px" style={{ marginTop: 14, background: '#fafbfc', display: 'flex', gap: 12, alignItems: 'flex-start' }}>
            <Icon name="info" size={14} color="#666"/>
            <div style={{ fontSize: 12, color: '#444', lineHeight: 1.6 }}>
              <strong>This is the only static, manual checklist in the program.</strong> No auto-grading, no AI completion. You mark each item because <em>you</em> are the one signing the certification — not a tool.
            </div>
          </Card>
        </div>

        {/* Pre-screen + textbook + submit */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <Card padding={0}>
            <div style={{ padding: '12px 16px', background: '#fafbfc', borderBottom: '1px solid #eee' }}>
              <span style={{ fontSize: 10, fontWeight: 800, color: '#1a9e5c', background: '#edfbf4', padding: '3px 8px', borderRadius: 4, letterSpacing: '.08em', textTransform: 'uppercase', marginRight: 8 }}>AI pre-screen</span>
              <span style={{ fontSize: 12.5, fontWeight: 700, color: '#292929' }}>Step 7 readiness</span>
            </div>
            <div style={{ padding: 14, display: 'flex', flexDirection: 'column', gap: 8 }}>
              {[
                ['URAR Form 1004 · 100% complete', 'All sections · narratives ≥ word floor'],
                ['Workfile · 14 artifacts archived', 'USPAP retention metadata attached'],
                ['Certification signed', 'Electronic signature affixed'],
                ['xAPI session log archived', '142 events captured'],
              ].map(([l, d], i) => (
                <div key={i} style={{ padding: 10, background: '#edfbf4', borderRadius: 6, border: '1px solid #1a9e5c33' }}>
                  <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                    <Icon name="check-circle" size={13} color="#1a9e5c"/>
                    <div style={{ fontSize: 12, fontWeight: 700, color: '#1a9e5c' }}>{l}</div>
                  </div>
                  <div style={{ fontSize: 11, color: '#444', marginTop: 3, paddingLeft: 21 }}>{d}</div>
                </div>
              ))}
            </div>
          </Card>

          <TextbookCallout topic="workfile" why="Re-read the Record Keeping Rule before you submit the workfile."/>

          <div style={{ background: '#1a1d2b', color: '#fff', borderRadius: 10, padding: 16 }}>
            <div style={{ fontSize: 10.5, color: '#cbd0e0', textTransform: 'uppercase', letterSpacing: '.05em', fontWeight: 700 }}>Async submission</div>
            <div style={{ fontSize: 13.5, fontWeight: 700, marginTop: 4 }}>Submit to James · 24 hr turnaround typical</div>
            <div style={{ fontSize: 11.5, color: '#cbd0e0', marginTop: 6, lineHeight: 1.5 }}>
              He'll evaluate against the same 4-point rubric you saw at MR 4 — then return scored feedback with textbook references for any gap.
            </div>
            <Button variant="primary" fullWidth style={{ marginTop: 12 }} onClick={() => navigate('client-sim')}>
              Continue to Step 8 · Client Communication <Icon name="arrow-right" size={13} color="#fff"/>
            </Button>
          </div>
        </div>
      </div>
...
function S32_ClientSim({ navigate }) {
  return (
    <div>
      <PageHeader breadcrumb={['Step 8', 'Section XXI']} title="Client communication sim — Maya is on the phone"
...
function S33_FinalReview({ navigate }) {
  return (
    <div>
      <PageHeader breadcrumb={['Step 8', 'Mentor Review 8']} title="Final Mentor Review — Report 1 complete"
        subtitle="Capstone review of all 8 steps. Mentor signs off. Report 1 lands in your portfolio."/>
      ...
      {/* Step audit summary — full width */}
      <Card padding="18px 22px" style={{ marginBottom: 14 }}>
        <div style={{ fontSize: 12, fontWeight: 700, color: '#888', textTransform: 'uppercase', letterSpacing: '.05em', marginBottom: 12 }}>All 8 steps · audit summary</div>
